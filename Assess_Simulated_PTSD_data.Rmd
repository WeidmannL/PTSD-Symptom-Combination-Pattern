...
Summary for two-combined-symptom removals sorted by newly non-diagnosed (100 two-symptoms-combinations with most lost diagnoses)
...
datatable(head(readable_most_lost_diagnoses_two_combined, 100),
          options = list(scrollX = TRUE))
```
...
variant1_analyze_best_six_symptoms_diagnostic_criteria <- function(data, score_by = "false_cases") {
  ...
  get_top_candidates <- function(data, cluster_indices, n_candidates) { # cluster_indices z.B. 1:5, n_candidates how many top candidates are chosen
    performances <- lapply(cluster_indices, function(symptom) {
      diagnoses <- data[, paste0("symptom_", symptom)]
      metrics <- get_metrics(diagnoses, baseline_results)
      score <- if(score_by == "false_cases") {
        -metrics$false_cases
      } else {
        -metrics$newly_nondiagnosed
      }
      list(symptom = symptom, score = score)
    })
    
    # Sort by score and take top n candidates
    sorted <- performances[order(sapply(performances, function(x) x$score), decreasing = TRUE)] # sapply() extracts score from each list element, order() gives indices that sort the score values from highest to lowest, performances[] reorders original list using this indices
    return(sapply(sorted[1:min(n_candidates, length(sorted))], function(x) x$symptom)) # sorted[] takes element 1:n, sapply() extracts just symptom number
  }
