...
fewest_lost_diagnoses_single <- summary_PTSD_Diagnosis_individual[order(summary_PTSD_Diagnosis_individual$newly_nondiagnosed),]
readable_fewest_lost_diagnoses_single <- create_readable_summary(fewest_lost_diagnoses_single)
datatable(readable_fewest_lost_diagnoses_single,
          options = list(scrollX = TRUE))
```

Summary for two-combined-symptom removals sorted by newly non-diagnosed (10 two-symptoms-combinations with fewest lost diagnoses)
...
fewest_lost_diagnoses_two_combined <- summary_PTSD_Diagnosis_combined_two %>%
...
readable_fewest_lost_diagnoses_two_combined <- create_readable_summary(fewest_lost_diagnoses_two_combined)
datatable(head(readable_fewest_lost_diagnoses_two_combined, 11),
          options = list(scrollX = TRUE))
```

Summary for three-combined-symptom removals sorted by newly non-diagnosed (10 three-symptoms-combinations with fewest lost diagnoses)
...
fewest_lost_diagnoses_three_combined <- summary_PTSD_Diagnosis_combined_three %>%
  # Filter for PTSD_all and combined deletions (containing two underscores)
  filter(column == "PTSD_all" | grepl("_.*_.*_", column)) %>%
  # Arrange by newly_nondiagnosed
  arrange(newly_nondiagnosed)
  # Convert to readable format
readable_fewest_lost_diagnoses_three_combined <- create_readable_summary(fewest_lost_diagnoses_three_combined)
datatable(head(readable_fewest_lost_diagnoses_three_combined, 11),
          options = list(scrollX = TRUE))
```
...
##### 5.3.3.3. Extension of the previous analysis: additional analysis of the impact of several simultaneously deleted symptoms on the diagnosis
