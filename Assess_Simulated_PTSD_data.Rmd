...
description_merged_binarized_possible_real_without_total <- function(data){
  ...
  description_matrix1 <- matrix(matrix_data1, ncol = 1, nrow = 6)
  rownames(description_matrix1) <- c("Number of theoretically possible combinations ", "Number of combinations that actually occur", 
  ...
  one_percent_individuals = round(nrow(data2)*0.01, 0)
  ...
  # Number of individuals reporting NOT one of the ten most common combinations only respecting combinations that actually occur
  ...
  # Number of individuals reporting one of the 50% least common combinations only respecting combinations that actually occur
  ...
  for (i in 1:n_symptoms) {
    ...
    symp_absolute_pat[i] <- sum(data2[,i] == 1) # Absolute prevalence of a symptom in patients
    symp_prevalence_pat[i] <- (symp_absolute_pat[i]/nrow(data2)) * 100 # Percentage prevalence of a symptom in patients
  }
  ...
  rownames(description_matrix3) <- colnames(data2)      
